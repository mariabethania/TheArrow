...
SoundFile pop1,pop2,pop3,pop1b,pop2b,pop3b,pop4,arrow1,ghost1,ghost2,ghost3;
...
void mousePressed() {
  if (menuBool && !scoreBool) {
    ...
    if (mouseX > width*0.2 && mouseX < width*0.45 && mouseY > height*0.7 && mouseY < height*0.75) {
      menuBool = true;
      scoreBool = false;
      //bs = new BalloonSystem();
      //ps = new PlopSystem();
...
void mouseDragged() {
  ...
  if ((mouseX < width*0.05 || mouseX > width*0.95) && mouseY < moveBow) {
    moveBow = mouseY+(height*0.03);
    bowTemp = mouseY+(height*0.03);
  }
  //if (mouseY < moveBow) {
  //moveBow = mouseY;
  //}
